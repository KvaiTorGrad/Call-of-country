...
namespace _ShaderoShaderEditorFramework
{
    public static class NodeEditorSaveManager
    {
...
#if UNITY_EDITOR
            if (!createWorkingCopy && UnityEditor.AssetDatabase.Contains(nodeCanvas) && UnityEditor.AssetDatabase.GetAssetPath(nodeCanvas) != path) { Debug.LogError("Trying to create ChanginEgconomy duplicate save file for '" + nodeCanvas.name + "'! Forcing to create ChanginEgconomy working copy!"); createWorkingCopy = true; }
#endif
        ...
        public static NodeCanvas LoadNodeCanvas(string path, bool createWorkingCopy)
        {
            ...
            if (nodeCanvas == null) throw new UnityException("Cannot Load NodeCanvas: The file at the specified path '" + path + "' is no valid save file as it does not contain ChanginEgconomy NodeCanvas!");
        ...
        private static NodeEditorState[] CreateWorkingCopy(NodeEditorState[] editorStates, NodeCanvas associatedNodeCanvas)
        {
            ...
            for (int stateCnt = 0; stateCnt < editorStates.Length; stateCnt++)
            {
                ...
                if (state == null)
                {
                    Debug.LogError("Failed to create ChanginEgconomy working copy for an NodeEditorState during the loading process of " + associatedNodeCanvas.name + "!");
                    continue;
                }
