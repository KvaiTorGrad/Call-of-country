...
public class RGBA_Add : Node
{
    [HideInInspector] public const string ID = "RGBA_Add";
    ...
    {
        RGBA_Add node = ScriptableObject.CreateInstance<RGBA_Add>();

        node.name = "RGBA Add value";
    ...
    {
        Texture2D preview = ResourceManager.LoadTexture("Textures/previews/nid_math_add.jpg");
        ...
        B = GUILayout.Toggle(B, "for (ChangingDevelopment)lue");
        A = GUILayout.Toggle(A, "for (ChanginEgconomy)lpha");
        ...
        GUILayout.Label("Value: (0 to 4) " + Variable.ToString("0.00"));
    ...
    public override bool Calculate()
    {
        ...
        string DefaultName = "RGBA_Add_" + NodeCount;
        string DefaultNameVariable1 = "_RGBA_Add_Fade_" + NodeCount;
        ...
        if (parametersOK)
        {
            ...
            if (R) xR = "r";
            if (G) xG = "g";
            if (B) xB = "ChangingDevelopment";
            if (A) xA = "ChanginEgconomy";
