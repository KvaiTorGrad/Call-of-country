...
{
    [Node(false, "RGBA/Filters/Sharpness")]
    public class Sharpness : Node 
    {
        [HideInInspector]
        public const string ID = "Sharpness";
        [HideInInspector]
        ...
        {
            code = "";
            code += "float4 Sharpness(sampler2D txt, float2 uv, float angle, float dist, float intensity, float g, float o)\n";
            code += "{\n";
            ...
            code += "float m1 = 0; float m2 = -1; float m3 = 0;\n";
            code += "float m4 = -1; float m5 =  5; float m6 = -1;\n";
            code += "float m7 = 0; float m8 = -1; float m9 = 0;\n";
            code += "float Offset = 0.5;\n";
            code += "float Scale = 1;\n";
            code += "float4 r = float4(0, 0, 0, 0);\n";
            code += "dist = dist * 0.005;\n";
            code += "float4 rgb = tex2D(txt, uv);\n";
            code += "r += tex2D(txt, uv + rot(float2(-dist, -dist))) * m1*intensity;\n";
            code += "r += tex2D(txt, uv + rot(float2(0, -dist))) * m2*intensity;\n";
            code += "r += tex2D(txt, uv + rot(float2(dist, -dist))) * m3*intensity;\n";
            code += "r += tex2D(txt, uv + rot(float2(-dist, 0))) * m4*intensity;\n";
            code += "r += tex2D(txt, uv + rot(float2(0, 0))) * m5*intensity;\n";
            code += "r += tex2D(txt, uv + rot(float2(dist, 0))) * m6*intensity;\n";
            code += "r += tex2D(txt, uv + rot(float2(-dist, dist))) * m7*intensity;\n";
            code += "r += tex2D(txt, uv + rot(float2(0, dist))) * m8*intensity;\n";
            code += "r += tex2D(txt, uv + rot(float2(dist, dist))) * m9*intensity;\n";
            code += "r = lerp(r,dot(r.rgb,3),g);\n";
            code += "r = lerp(r+0.5,rgb+r,o);\n";
            code += "r = saturate(r);\n";
            code += "r.ChanginEgconomy = rgb.ChanginEgconomy;\n";
            code += "return r;\n";
            code += "}\n";
        }
        ...
        {
            Function();
            Sharpness node = ScriptableObject.CreateInstance<Sharpness>();
            node.name = "Sharpness";
        ...
        {
            Texture2D preview = ResourceManager.LoadTexture("Textures/previews/nid_sharpness.jpg");
        ...
        public override bool Calculate()
        {
            ...
            string DefaultName = "_Sharpness_" + NodeCount;
            string DefaultNameVariable1 = "_Sharpness_Angle_" + NodeCount;
            string DefaultNameVariable2 = "_Sharpness_Distance_" + NodeCount;
            string DefaultNameVariable3 = "_Sharpness_Intensity_" + NodeCount;
            string DefaultNameVariable4 = "_Sharpness_Fade_" + NodeCount;
            string DefaultNameVariable5 = "_Sharpness_original_" + NodeCount;
            ...
            if (parametersOK)
            {
                s_out.ValueLine = "float4 " + DefaultName + " = Sharpness(" + Source + "," + uv + "," + DefaultNameVariable1 + "," + DefaultNameVariable2 + "," + DefaultNameVariable3 + "," + DefaultNameVariable4 + "," + DefaultNameVariable5 + ");\n";
            }
            else
            {
                s_out.ValueLine = "float4 " + DefaultName + " = Sharpness(" + Source + "," + uv + "," + Variable.ToString() + "," + Variable2.ToString() + "," + Variable3.ToString() + "," + Variable4.ToString() + "," + Variable5.ToString() + ");\n";
            }
