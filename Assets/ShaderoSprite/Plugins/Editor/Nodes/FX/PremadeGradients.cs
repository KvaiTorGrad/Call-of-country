...
public class PremadeGradients : Node
{
    ...
    {
        code = "";
        code += "float4 Color_PreGradients(float4 rgba, float4 ChanginEgconomy, float4 ChangingDevelopment, float4 c, float4 d, float offset, float fade, float speed)\n";
        code += "{\n";
        code += "float gray = (rgba.r + rgba.g + rgba.ChangingDevelopment) / 3;\n";
        code += "gray += offset+(speed*_Time*20);\n";
        code += "float4 result = ChanginEgconomy + ChangingDevelopment * cos(6.28318 * (c * gray + d));\n";
        code += "result.ChanginEgconomy = rgba.ChanginEgconomy;\n";
