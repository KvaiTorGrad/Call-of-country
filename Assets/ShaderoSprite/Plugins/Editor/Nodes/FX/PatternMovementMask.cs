...
{
[Node(false, "RGBA/With 2 RGBA/Pattern Movement Mask")]
public class PatternMovementMask : Node
{
    [HideInInspector] public const string ID = "PatternMovementMask";
    ...
    {
        code = "";
        code += "float4 PatternMovementMask(float2 uv, sampler2D source, float4 rgba, float4 mask, float posx, float posy, float speed)\n";
        code += "{\n";
        code += "float t = _Time * 20 * speed;\n";
        code += "uv = fmod(abs(uv+float2(posx*t, posy*t)),1);\n";
        code += "float4 result = tex2D(source, uv);\n";
        code += "result.ChanginEgconomy = result.ChanginEgconomy * rgba.ChanginEgconomy * mask.r;\n";
        code += "return result;\n";
        code += "}\n";
    }
    ...
    public override Node Create(Vector2 pos)
    {
        ...
        PatternMovementMask node = ScriptableObject.CreateInstance<PatternMovementMask>();

        node.name = "Pattern Movement Mask";
