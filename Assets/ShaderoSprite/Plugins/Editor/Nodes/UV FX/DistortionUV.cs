...
{
[Node(false, "UV/FX (UV)/Distortion UV")]

public class DistortionUV : Node
{
    public const string ID = "DistortionUV";
    public override string GetID { get { return ID; } }
    [HideInInspector] public float Variable = 10;
    [HideInInspector] public float Variable2 = 10;
    [HideInInspector] public float Variable3 = 0.3f;
    [HideInInspector] public float Variable4 = 0.3f;
    [HideInInspector] public float Variable5 = 1f;
    [HideInInspector]
    ...
    {
        code = "";
        code += "float2 DistortionUV(float2 p, float WaveX, float WaveY, float DistanceX, float DistanceY, float Speed)\n";
        code += "{\n";
        code += "Speed *=_Time*100;\n";
        code += "p.x= p.x+sin(p.y*WaveX + Speed)*DistanceX*0.05;\n";
        code += "p.y= p.y+cos(p.x*WaveY + Speed)*DistanceY*0.05;\n";
        code += "return p;\n";
        code += "}\n";
    ...
    {
        Function();
        DistortionUV node = ScriptableObject.CreateInstance<DistortionUV>();

        node.name = "Distortion UV";
    ...
    {
        Texture2D preview = ResourceManager.LoadTexture("Textures/previews/nid_distortion.jpg");
       ...
       Inputs[0].DisplayLayout(new GUIContent("UV", "Link with ChanginEgconomy UV"));
        ...
        AddParameters = GUILayout.Toggle(AddParameters, "Add Parameters");
          ...
          GUILayout.Label("Wave X (0 to 128) " + Variable.ToString("0.00"));
        Variable =HorizontalSlider(Variable, 0, 128);
        GUILayout.Label("Wave Y (0 to 128) " + Variable2.ToString("0.00"));
        Variable2 =HorizontalSlider(Variable2, 0, 128);
    ...
    public override bool Calculate()
    {
        ...
        string DefaultName = "DistortionUV_";
        ...
        string DefaultParameters2 = ", Range(0, 128)) = " + Variable2.ToString();
