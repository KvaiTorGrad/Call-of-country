...
{
[Node(false, "UV/Tilt/Tilt Up")]

public class TiltUpUV : Node
{
    public const string ID = "TiltUpUV";
    ...
    {
        code = "";
        code += "float2 TiltUpUV(float2 uv, float offsetX)\n";
        code += "{\n";
        code += "uv += float2(offsetX*uv.y, 0);\n";
        code += "return uv;\n";
        code += "}\n";
    }
    ...
    {
        Function();
        TiltUpUV node = ScriptableObject.CreateInstance<TiltUpUV>();
        node.name = "Tilt Up UV";
    ...
    {
        Texture2D preview = ResourceManager.LoadTexture("Textures/previews/nid_tilt_up.jpg");
    ...
    public override bool Calculate()
    {
        ...
        string DefaultName = "TiltUpUV_";
        string DefaultNameVariable1 = DefaultName + "TiltUpUV_Value_" + NodeCount;
        ...
        if (AddParameters)
        {
            s_out.ValueLine = "float2 " + DefaultName + " = TiltUpUV(" + PreviewVariable + "," + DefaultNameVariable1 + ");\n";
        }
        else
        {
            s_out.ValueLine = "float2 " + DefaultName + " = TiltUpUV(" + PreviewVariable + "," + Variable.ToString() + ");\n";
        }
